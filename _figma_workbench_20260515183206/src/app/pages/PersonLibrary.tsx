...
export function PersonLibrary({ isMobile, onOpenDrawer, showToast }: PersonLibraryProps) {
        ...
        >
          <div className="flex flex-col rounded-2xl overflow-hidden" style={{ width: 480, maxWidth: "calc(100vw - 16px)", background: "#FAFAF8", boxShadow: "0 24px 64px rgba(0,0,0,0.18)" }}>
            ...
            <div className="flex justify-end gap-3 px-6 py-4" style={{ borderTop: "1px solid #EDE9DF" }}>
              ...
              <button onClick={() => { if (batchImportFile && batchImportFile.length > 0) { setBatchImportOpen(false); showToast(`已导入 ${batchImportFile.length} 张图片`); setBatchImportFile(null); } else { imgInputRef.current?.click(); } }} className="px-5 py-2 rounded-full" style={{ background: "#F4C542", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#141414" }}>
        ...
        >
          <div className="flex flex-col rounded-2xl overflow-hidden" style={{ width: 480, maxWidth: "calc(100vw - 16px)", background: "#FAFAF8", boxShadow: "0 24px 64px rgba(0,0,0,0.18)" }}>
            ...
            <div className="flex justify-end gap-3 px-6 py-4" style={{ borderTop: "1px solid #EDE9DF" }}>
              ...
              <button onClick={() => { if (tableImportFile) { setImportTableOpen(false); showToast(`已导入「${tableImportFile.name}」`); setTableImportFile(null); } else { tableInputRef.current?.click(); } }} className="px-5 py-2 rounded-full" style={{ background: "#F4C542", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#141414" }}>
