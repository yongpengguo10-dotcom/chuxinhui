...
import { effectiveStatus, getDependencyTasks, isBlocked, isDoneStatus, isOverdue, statusStyle } from "../lib/taskUtils";
...
export function RoleWorkspace({
  ...
  onSwitchProject, onUpdateTask, onCreateTask, onAddProjectImage, isMobile, onOpenDrawer, showToast
}: RoleWorkspaceProps) {
  const [filterStatus, setFilterStatus] = useState<"全部" | "等待前置任务" | "未开始" | "进行中" | "待审核" | "已完成">("全部");
  ...
  const stats = useMemo(() => {
    ...
    const done = roleTasks.filter(t => isDoneStatus(effectiveStatus(t, tasks))).length;
    const review = roleTasks.filter(t => effectiveStatus(t, tasks) === "待审核").length;
  ...
  const filteredTasks = useMemo(() => {
    return roleTasks.filter(t => {
      if (filterStatus !== "全部") {
        if (filterStatus === "已完成") {
          if (!isDoneStatus(effectiveStatus(t, tasks))) return false;
        } else {
          if (effectiveStatus(t, tasks) !== filterStatus) return false;
        }
      }
      if (searchQuery) {
  ...
  }, [roleTasks, filterStatus, searchQuery, tasks]);
  ...
  const handleSubmitFile = (file: File | undefined) => {
    if (!file) return;
    const reader = new FileReader();
    reader.onload = e => {
      setSubmitFileName(file.name);
      setSubmitDataUrl(String(e.target?.result ?? ""));
    };
  ...
  const confirmAction = () => {
    ...
    if (type === "upload") {
      if (isBlocked(task, tasks)) {
        showToast("前置任务未完成，暂时不能提交成果");
        return;
      }
      if (task.resultType === "图片" && !submitDataUrl) {
        showToast("请先上传图片成果");
        return;
      }
      if (task.resultType === "文案" && !submitNote.trim() && !submitLink.trim()) {
        showToast("请填写文案内容或协作链接");
        return;
      }
      onUpdateTask(task.id, {
        uploaded: true,
        status: task.needReview ? "待审核" : "已完成",
        resultContent: task.resultType === "文案" ? submitNote.trim() : undefined,
        resultLink: submitLink.trim() || undefined,
        resultImageUrl: task.resultType === "图片" ? submitDataUrl : undefined,
        resultFileName: submitFileName || undefined,
        resultNote: task.resultType === "文案" ? undefined : submitNote.trim() || undefined,
      ...
      if (task.resultType === "图片" || task.resultType === "视频") {
        if (onAddProjectImage) {
          const newImg = {
            ...
            versions: [{
              id: "v1",
              url: task.resultType === "图片" ? submitDataUrl : "https://images.unsplash.com/photo-1517048676732-d65bc937f952?auto=format&fit=crop&q=80&w=600",
              versionStr: "V1",
      ...
      <div className="flex flex-col flex-1 rounded-2xl" style={{ background: "#FFFFFF", border: "1px solid #EDE9DF", minHeight: 400 }}>
        ...
        <div className={`p-4 border-b border-[#EDE9DF] flex ${isMobile ? "flex-col gap-3" : "items-center justify-between"}`}>
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1" style={{ scrollbarWidth: "none" }}>
            ...
            <FilterChip label="等待前置" active={filterStatus === "等待前置任务"} onClick={() => setFilterStatus("等待前置任务")} />
          ...
          {filteredTasks.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {filteredTasks.map(task => (
                <TaskRow key={task.id} task={task} allTasks={tasks} onUpload={() => handleUploadResult(task)} onComplete={() => handleCompleteTask(task)} isMobile={isMobile} />
              ))}
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-[#141414]/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-5 flex flex-col gap-4">
              ...
              <DependencyPanel task={actionTask.task} tasks={tasks} />

              {actionTask.type === "upload" && actionTask.task.resultType === "图片" ? (
                <>
                  <div
                    className="border-2 border-dashed border-[#D0C8B0] rounded-xl p-6 flex flex-col items-center justify-center gap-2 bg-[#FAFAF8] hover:bg-[#FFFDF0] hover:border-[#F4C542] transition-colors cursor-pointer"
                    onClick={() => submitFileInputRef.current?.click()}
                    onDragOver={e => e.preventDefault()}
                    onDrop={e => {
                      e.preventDefault();
                      handleSubmitFile(e.dataTransfer.files?.[0]);
                    }}
                  >
                    <div className="w-10 h-10 rounded-full bg-[#FFF9E6] flex items-center justify-center text-[#8A6500]">
                      <UploadCloud size={20} />
                    </div>
                    <div className="text-[13px] font-bold text-[#141414]">{submitFileName || "点击或拖拽上传图片"}</div>
                    <div className="text-[11px] text-[#999]">支持 JPG, PNG, WebP，不超过 20MB</div>
                  </div>
                  <input
              ...
              ) : actionTask.type === "upload" && actionTask.task.resultType === "文案" ? (
                <div className="flex flex-col gap-2">
                  <label className="text-[12px] font-bold text-[#555] flex items-center gap-1">
                    <FileText size={12} /> 文案内容
                  </label>
                  <textarea
                    value={submitNote}
                    onChange={e => setSubmitNote(e.target.value)}
                    placeholder="把最终文案粘贴在这里，设计岗位会直接看到这份内容..."
                    className="w-full px-3 py-2 rounded-lg border border-[#EDE9DF] text-[13px] outline-none focus:border-[#F4C542] focus:bg-[#FFFDF0] transition-colors resize-none h-28"
                  />
                  <input 
                    value={submitLink}
                    onChange={e => setSubmitLink(e.target.value)}
                    placeholder="也可以补充飞书/石墨等协作链接"
...
function DependencyPanel({ task, tasks }: { task: Task; tasks: Task[] }) {
  const deps = getDependencyTasks(task, tasks);
  if (deps.length === 0) return null;
  return (
    <div className="rounded-xl p-3" style={{ background: "#F8F6EF", border: "1px solid #EDE9DF" }}>
      <div className="text-[12px] font-bold text-[#555] mb-2">前置任务与可用成果</div>
      <div className="flex flex-col gap-2">
        {deps.map(dep => {
          const done = isDoneStatus(dep.status);
          return (
            <div key={dep.id} className="rounded-lg p-2" style={{ background: "#FFFFFF", border: `1px solid ${done ? "#A8E1BD" : "#D7DFEA"}` }}>
              <div className="flex items-center justify-between gap-2 mb-1">
                <span className="text-[12px] font-bold text-[#141414]">{dep.name}</span>
                <span className="text-[10px] font-bold px-1.5 py-0.5 rounded" style={{ background: done ? "#E9F8EF" : "#F5F7FA", color: done ? "#1E7A3D" : "#52616F" }}>
                  {done ? "已完成" : "未完成"}
                </span>
              </div>
              {(dep.resultContent || dep.resultLink) ? (
                <div className="text-[12px] text-[#555] leading-relaxed whitespace-pre-wrap">{dep.resultContent || dep.resultLink}</div>
              ) : (
                <div className="text-[11px] text-[#999]">暂未提交成果</div>
              )}
            </div>
          );
        })}
...
function TaskRow({ task, allTasks, onUpload, onComplete, isMobile }: { task: Task; allTasks: Task[]; onUpload: () => void; onComplete: () => void; isMobile: boolean }) {
  const status = effectiveStatus(task, allTasks);
  const s = statusStyle(status);
  const overdue = isOverdue(task);
  const dependencies = getDependencyTasks(task, allTasks);
        ...
        {dependencies.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1.5">
            {dependencies.map(dep => {
              const done = isDoneStatus(dep.status);
              return (
                <span key={dep.id} className="px-2 py-0.5 rounded-full text-[11px]" style={{
                  background: done ? "#E9F8EF" : "#F5F7FA",
                  border: `1px solid ${done ? "#A8E1BD" : "#D7DFEA"}`,
                  color: done ? "#1E7A3D" : "#52616F",
                }}>
                  前置：{dep.name}{done ? " 已完成" : " 未完成"}
                </span>
              );
            })}
          </div>
        )}
        {dependencies.some(dep => dep.resultContent || dep.resultLink) && (
          <div className="mt-2 rounded-lg p-2" style={{ background: "#FFFFFF", border: "1px solid #EDE9DF" }}>
            <div style={{ fontSize: 11, color: "#777", fontWeight: 700, marginBottom: 4 }}>可引用的前置成果</div>
            {dependencies.filter(dep => dep.resultContent || dep.resultLink).map(dep => (
              <div key={dep.id} style={{ fontSize: 12, color: "#141414", lineHeight: 1.5, whiteSpace: "pre-wrap" }}>
                {dep.resultContent || dep.resultLink}
              </div>
            ))}
          </div>
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        {task.needUpload && !task.uploaded && status !== "已完成" && status !== "已定稿" && (
          <button
            onClick={onUpload}
            disabled={blocked}
            className="flex items-center gap-1 px-3 py-1.5 rounded-full transition-colors text-[12px] font-bold"
            style={{
              background: blocked ? "#F0EDE4" : "#F4C542",
              color: blocked ? "#999" : "#141414",
              cursor: blocked ? "not-allowed" : "pointer",
            }}
          >
            <UploadCloud size={14} /> {blocked ? "等待前置" : "提交成果"}
          </button>
        )}
