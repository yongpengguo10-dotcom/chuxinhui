...
export function ProjectImageLibrary({ projects, images, setImages, isMobile, onOpenDrawer, showToast }: ProjectImageLibraryProps) {
  ...
  const displayImages = useMemo(() => {
    let list = latestLibraryImages(images);
  ...
  const taskFolderGroups = useMemo(() => {
    const groups = new Map<string, ProjectImage[]>();
    displayImages.forEach(image => {
      const folder = image.taskFolderName || "未归档图片";
      groups.set(folder, [...(groups.get(folder) ?? []), image]);
    });
  ...
  const handleUploadFile = (file: File | undefined) => {
    ...
    reader.onload = e => {
      ...
      setUploadDataUrl(String(e.target?.result ?? ""));
      if (!newImageTitle.trim()) setNewImageTitle(file.name.replace(/\.[^.]+$/, ""));
    };
        ...
        <div className="flex-1 overflow-y-auto p-6" style={{ scrollbarWidth: "thin" }}>
          {displayImages.length === 0 ? (
            ...
            <div className="flex flex-col gap-8">
              {taskFolderGroups.map(([folderName, folderImages]) => (
                <section key={folderName}>
                  <div className="flex items-center gap-2 mb-3 text-sm font-bold text-gray-900">
                    <Folder size={15} color="#8A6500" />
                    <span>{folderName}</span>
                    <span className="text-xs font-medium text-gray-400">{folderImages.length} 张</span>
                  </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-5 flex flex-col gap-4">
              ...
              >
                ...
                <div className="text-[13px] font-bold text-[#141414]">{uploadFileName || "点击或拖拽上传文件"}</div>
              ...
              />
              {uploadDataUrl && (
                <div className="aspect-video rounded-xl overflow-hidden border border-[#E8E3D8] bg-[#F8F6EF]">
                  <img src={uploadDataUrl} alt="上传预览" className="w-full h-full object-contain" />
                </div>
              )}
            ...
            <div className="p-4 border-t border-[#EDE9DF] flex justify-end gap-2 bg-[#FAFAF8]">
              <button 
                onClick={() => { setUploadModalOpen(false); resetUploadDraft(); }}
