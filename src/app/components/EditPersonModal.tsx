...
export function EditPersonModal({ person, categories, onSave, onCancel, onDelete }: EditPersonModalProps) {
  ...
  const [imgOffsetX, setImgOffsetX] = useState(person?.imageOffsetX ?? 0);
  const [imgOffsetY, setImgOffsetY] = useState(person?.imageOffsetY ?? 0);
  const [imgScale, setImgScale] = useState(person?.imageScale ?? 100);
  const [photos, setPhotos] = useState<string[]>(person?.photos ?? []);
  ...
  const handleSave = () => {
    ...
    const saved: Person = {
      ...
      currentImageIndex: photos.length > 0 ? 0 : person?.currentImageIndex ?? 0,
      avatarIndex,
      photos,
  ...
  const handleImageFiles = (incoming: FileList | null) => {
    if (!incoming) return;
    Array.from(incoming).filter(file => file.type.startsWith("image/")).forEach(file => {
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {photos.map((photo, index) => (
                    <button
                      ...
                      style={{ width: 44, height: 52, border: index === 0 ? "2px solid #F4C542" : "1px solid #E8E3D8", cursor: "pointer", padding: 0 }}
                      title="点击删除"
                    >
                      <PersonAvatar name={name || "人"} avatarIndex={avatarIndex} src={photo} size="thumb" />
                    </button>
                  ))}
                </div>
              )}
