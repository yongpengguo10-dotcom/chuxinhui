...
interface PersonDetailProps {
  ...
  onUpdate: (person: Person, message?: string) => void;
  onClose: () => void;
}

export function PersonDetail({ person, onEdit, onAI, onUpdate, onClose }: PersonDetailProps) {
  const [currentImg, setCurrentImg] = useState(person.currentImageIndex ?? 0);
  ...
  const imageCount = Math.max(person.imageCount, photos.length, 1);
  const currentPhoto = photos[currentImg];

  useEffect(() => {
    setCurrentImg(Math.min(person.currentImageIndex ?? 0, Math.max((person.photos?.length ?? 1) - 1, 0)));
  }, [person.id, person.currentImageIndex, person.photos?.length]);

  const handleSetMainImg = () => {
    onUpdate({ ...person, currentImageIndex: currentImg }, `已设第 ${currentImg + 1} 张为主图`);
  ...
  const handleAddPhotos = (newPhotos: string[]) => {
    ...
    onUpdate(
      { ...person, photos: nextPhotos, imageCount: Math.max(nextPhotos.length, 1), currentImageIndex: person.photos?.length ? person.currentImageIndex : 0 },
      `已上传 ${newPhotos.length} 张图片`
    );
  };
  ...
  const handleDeletePhoto = (index: number) => {
    ...
    const nextIndex = nextPhotos.length === 0 ? 0 : Math.min(currentImg, nextPhotos.length - 1);
    setCurrentImg(nextIndex);
    onUpdate(
      { ...person, photos: nextPhotos, imageCount: Math.max(nextPhotos.length, 1), currentImageIndex: nextIndex },
      "已删除图片"
    );
  };
  ...
  const handleDownloadCurrent = () => {
    ...
    link.download = `${person.name}-图片${currentImg + 1}.png`;
    link.click();
  };
  ...
  const quickActions = [
    ...
    { icon: Download, label: "保存图片", onClick: handleDownloadCurrent },
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2" style={{ scrollbarWidth: "thin" }}>
            {Array.from({ length: person.imageCount }).map((_, i) => (
              <div
                ...
                onClick={() => setCurrentImg(i)}
              >
                <PersonAvatar name={person.name} avatarIndex={person.avatarIndex} src={photos[i]} size="thumb" />
                <div
...
function UploadModal({ personName, onClose, onUpload }: { personName: string; onClose: () => void; onUpload: (photos: string[]) => void }) {
