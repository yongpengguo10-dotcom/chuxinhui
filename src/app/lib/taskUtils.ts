...
export const STATUS_STYLES: Record<TaskStatus, { bg: string; border: string; text: string; dot: string }> = {
  "等待前置任务": { bg: "#F5F7FA", border: "#D7DFEA", text: "#52616F", dot: "#7D8DA1" },
...
export const DEFAULT_ROLE_COLOR = { bg: "#F5F7FA", border: "#D7DFEA", text: "#52616F" };
export const DEFAULT_STATUS_STYLE = { bg: "#F5F7FA", border: "#D7DFEA", text: "#52616F", dot: "#7D8DA1" };
export const DEFAULT_PRIORITY_STYLE = { bg: "#F8F6EF", border: "#E8E3D8", text: "#777" };

export function roleColor(role: string) {
  return ROLE_COLOR[role as TaskRole] ?? DEFAULT_ROLE_COLOR;
}

export function statusStyle(status: string) {
  return STATUS_STYLES[status as TaskStatus] ?? DEFAULT_STATUS_STYLE;
}

export function priorityStyle(priority: string) {
  return PRIORITY_STYLES[priority as TaskPriority] ?? DEFAULT_PRIORITY_STYLE;
}

export function isOverdue(task: Task): boolean {
  if (task.status === "已完成" || task.status === "已定稿") return false;
  if (task.status === "等待前置任务") return false;
  return daysUntil(task.deadline) < 0;
}

export function isDoneStatus(status: TaskStatus): boolean {
  return status === "已完成" || status === "已定稿";
}
...
export function getBlockingDependencies(task: Task, allTasks: Task[] = []): Task[] {
  return getDependencyTasks(task, allTasks).filter(t => !isDoneStatus(t.status));
}

export function isBlocked(task: Task, allTasks: Task[] = []): boolean {
  if (task.status === "已完成" || task.status === "已定稿" || task.status === "待审核") return false;
  return getBlockingDependencies(task, allTasks).length > 0;
}

export function effectiveStatus(task: Task, allTasks: Task[] = []): TaskStatus {
  if (isBlocked(task, allTasks)) return "等待前置任务";
...
export function projectStats(tasks: Task[], projectId: string) {
  ...
  const blocked = list.filter(t => isBlocked(t, list)).length;
